    ...
    for(int row = 0; row < BOARD_HEIGHT; row++){          
        ...
        for(int col = 0; col<BOARD_WIDTH; col++){
          ...
          if(playerdata.returnGridData()->board[row][col].returnShot()) { //Changed some variables into public for access
              if(playerdata.returnGridData()->board[row][col].returnShip() != 0){ //If the cell has a ship
                  cellsymbol = Hit;          
              }
              ...
              if(playerdata.returnGridData()->board[row][col].returnShip() != 0){ //If the cell has a ship
                  switch(playerdata.returnGridData()->board[row][col].returnShip()){
                      case 1 : cellsymbol = Destroyer; break;
                      case 2 : cellsymbol = Submarine; break;
                      case 3 : cellsymbol = Cruiser; break;
                      case 4 : cellsymbol = Battleship; break;
                      case 5 : cellsymbol = Aircraft_Carrier; break; 
                  };
              }
              else{ //If the cell doesn't have a ship
                  cellsymbol = Empty;           
...
void View::badfit(){
  cout<<"Your ship doesn't fit! Try again: "<<endl;
}
